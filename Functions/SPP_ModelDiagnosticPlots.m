...
% TransformedTimes -> A vector containing the transformed times \tau_k for 
%                     the time series. 
...
function SPP_ModelDiagnosticPlots(mu,Topic,AIC,TransformedTimes,U)
...
t = suptitle(sprintf('Topic %d Fitted with a Stationary Poisson Process \n with $\\hat{\\mu}$ = %.3e and AIC = %.3e',Topic,mu,AIC));
set(t,'Interpreter','Latex','Fontsize',18);

saveas(gcf, strcat('SPP_Results/SPP_MDP/SPP_MDP_Topic_',num2str(Topic)), 'tiff'); 
saveas(gcf, strcat('SPP_Results/SPP_MDP/SPP_MDP_Topic_',num2str(Topic)), 'epsc'); 
saveas(gcf, strcat('SPP_Results/SPP_MDP/SPP_MDP_Topic_',num2str(Topic)), 'fig'); 